...
After joining the data sheets, we have 292 species that we are working with. 
...
phlox_clean_up1 <- phlox_merge %>%
  mutate_all(funs(str_replace(., " or", ", "))) %>% #mutations 1-4 are used to ensure all cells with more than one value are using the same separators, a comma
...
Adding genus column data: #Do we need "genus_name <-" ?? 
```{r}
...
phlox_long %>%
  ...
  scale_fill_manual(values = c("blue_purple" = "slateblue",
                               ...
                               "white" = "white",
                               "yellow" = "gold")) +
  labs(title = "Color Count", x = "Corolla Color", y = "Number of Phlox Species")+
...
##Pollinator Analysis WHY IS THERE A POLLINATOR WITH ""?
```{r}
phlox_long %>% 
...
phlox_long %>% 
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1),
        plot.title=element_text(size = rel(1.5), face="bold", hjust=.5))
```
#Let's still do plots comparing color to corolla length:width ratio, and comparing pollinators to corolla length:width ratio
```{r}
phlox_long %>% 
